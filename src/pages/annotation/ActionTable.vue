    ...
    <template v-slot:body="props">
      <q-tr :class="{ 'bg-warning': props.row.end - props.row.start <= 0 }">
        ...
        <q-td>
          <img
            v-if="
              configurationStore.actionLabelData.find(
                (label) => label.id === props.row.action
              ).thumbnail
            "
          ...
          <q-select
            ...
            :options="filteredActionObjectList"
            dense
            options-dense
            borderless
            emit-value
        ...
        <q-td auto-width>
          <q-btn-group spread flat>
            ...
            >
              <q-tooltip
                >Set current left / right frame as this action's start /
                end</q-tooltip
              >
            </q-btn>
            <q-btn
...
const handleThumbnailPreview = (src) => {
  mainStore.currentActionThumbnailSrc =
    mainStore.currentActionThumbnailSrc === src ? null : src
}
const handleActionInput = (row) => {
  row.color = configurationStore.actionLabelData.find(
    (label) => label.id === row.action
  ).color
  row.object = configurationStore.actionLabelData.find(
    (label) => label.id === row.action
  ).objects[0]
}
